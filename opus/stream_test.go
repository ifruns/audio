...
// Extract raw pcm data from .pcm file
func extractWavPcm(t *testing.T, fname string) []int16 {
	...
	if (len(bytes)-wavHeaderSize)%2 == 1 {
		t.Fatalf("Illegal pcm data: payload must be encoded in byte pairs")
	}
...
func TestStream(t *testing.T) {
	...
	wavpcm := extractWavPcm(t, "testdata/speech_8.pcm")
	if len(opuspcm) != len(wavpcm) {
		t.Fatalf("Unexpected length of decoded opus file: %d (.pcm: %d)", len(opuspcm), len(wavpcm))
	}
