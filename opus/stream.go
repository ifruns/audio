...
// Stream wraps a io.OggReader in a decoding layer. It provides an API similar to
// io.OggReader, but it provides raw PCM data instead of the encoded Opus data.
//
// This is not the same as directly decoding the bytes on the io.OggReader; opus
...
// Init initializes a stream with an io.OggReader to fetch opus encoded data from
...
func (s *Stream) Init(read io.Reader) error {
	...
	if read == nil {
		return fmt.Errorf("OggReader must be non-nil")
	}
