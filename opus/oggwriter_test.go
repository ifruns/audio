...
func TestOggWriter_AddPacketAndClose(t *testing.T) {
	...
	writer, err := OpenWriter(addPacketTestCase[0].buffer, 48000, 2)
	...
	writer, err = OpenWriter(addPacketTestCase[1].buffer, 48000, 2)
	assert.Nil(err, "OggWriter should be created")
	assert.NotNil(writer, "OggWriter shouldn't be nil")
	addPacketTestCase[1].writer = writer

	// Third test writes tries to write a valid Opus packet
	writer, err = OpenWriter(addPacketTestCase[2].buffer, 48000, 2)
	assert.Nil(err, "OggWriter should be created")
	assert.NotNil(writer, "OggWriter shouldn't be nil")
	addPacketTestCase[2].writer = writer

	// Fourth test tries to write to a nil stream
	writer, err = OpenWriter(addPacketTestCase[3].buffer, 4800, 2)
