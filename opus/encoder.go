...
func NewEncoder(sampleRate int, channels int, application Application) (*Encoder, error) {
	var enc Encoder
	err := enc.Init(sampleRate, channels, application)
	if err != nil {
		return nil, err
	}
	return &enc, nil
...
func (enc *Encoder) Init(sampleRate int, channels int, application Application) error {
	if enc.p != nil {
	...
	if cap(enc.mem) < int(size) {
		enc.mem = make([]byte, size)
	} else {
		// memset to 0
		enc.mem = enc.mem[:size]
		if len(_empty) < len(enc.mem) {
			count := len(enc.mem) / len(_empty)
			for i := 0; i < count; i++ {
				copy(enc.mem[i*len(_empty):i*len(_empty)+len(_empty)], _empty[:])
			}
			last := len(enc.mem) % len(_empty)
			if last > 0 {
				copy(enc.mem[count*len(_empty):], _empty[:last])
			}
		} else {
			copy(enc.mem, _empty[:len(enc.mem)])
		}
	}
