...
func (w *OggWriter) writePage(payload []byte, headerType uint8, granulePos uint64) error {
	if w.stream == nil {
		return os.ErrClosed
	}
...
func (w *OggWriter) Flush() error {
	w.mu.Lock()
	defer w.mu.Unlock()
	return w.flushPage()
}

func (w *OggWriter) flushPage() error {
	if w.writerIndex == 0 {
		return nil
	}
...
func (w *OggWriter) flushEOF() error {
	return w.writePage(
		w.pageBuffer[:w.writerIndex],
...
func (w *OggWriter) writeSegments(packet []byte, samples int) error {
	if len(packet) == 0 {
		return nil
	}
...
func (w *OggWriter) Write(packet []byte, samples int) error {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *OggWriter) WriteEOF(packet []byte, samples int) error {
	w.mu.Lock()
	defer w.mu.Unlock()
